...
/+ For each member of an object, fetch the IUP interface element of the same name and assign it
 class MyWidgets {
   IupWidget widget1, widget2;
 };
 MyWidgets w;
 FetchWidgets(w); // finds "widget1", "widget2" from IUP
+/
void FetchWidgets(Class)(ref Class obj) {

    // for each member of the class, get the corresponding widget
    foreach (memberName; __traits(derivedMembers, Class)) {

        // get 'handle' on interface element by name
        auto helement = IupGetHandle(memberName.toStringz);

        // create IupWidget wrapper if we found the interface element
        IupWidget widget;
        if (helement !is null)
            widget = new IupWidget(helement, No.Owner);

        // assign the results to the object member
        __traits(getMember, obj, memberName) = widget;
    }
}
